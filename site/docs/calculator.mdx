...
import Toml              from '!!raw-loader!../../gradle/libs.versions.toml';
...
The `build.gradle.kts` file defines how the app is configured and all its dependencies. In our case, we will use a multi-plaform configuration so our app can run on all Doodle supported targets.
...
:::info
The gradle build uses [libs.versions.toml](https://github.com/nacular/doodle-tutorials/blob/master/gradle/libs.versions.toml) file.
:::

### Source Files

All source code and resources are located under the `src` directory. The application logic itself is located in the common source set (`src/commonMain`), which means it is entirely reused for each platform. In fact, the same app is used unchanged (just targeting JS) within this documentation. Font files are located in `src/commonMain/resources`. Then we have the platform-specific launch code in `src/jsMain`, `src/wasmJsMain` and `src/jvmMain`. Lastly, there are some app tests written for the JVM in `src/jvmTest`.
